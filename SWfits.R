...
swchecks <- function(vars,dat,obsvar="rt",xqnts=NA,pxqnts=NA,method=1,qqplot=3,xliml=7,frceaxis=FALSE,doqq=TRUE,dodist=TRUE,ylimlqq=0,ylimuqq=0,sim,xlimudec=0,ylimudec=0,ylimlcel=0,ylimucel=0,frame=FALSE){
  ...
  points(mxqnts,mpxqnts,pch=19,bg="red",col="red")
  }
  box()
...
##################
## Example Application 1: Simulated Data

## 1. Load the data 
##    Note that the data file should be located in your "getwd()" folder 
##    The data should be in data.frame format 
##    Make sure the data don't have NA or negative values

##    AAV15_mydata.csv is an example simulated data set (with minor noise added)
...
f <- swapply(dat=d,search=TRUE, obsvar="RT", #name of RT variable
             facs=c("Attention","RDK","Subj"))  #design cell combos to fit

##    Alternatively, fit by a specified range which runs faster but might be less precise
##    ql=.001, qu=.999 is recommended for simulated or non-noisy data
##    ql=.01, qu=.99 is generally recommended for real data

f <- swapply(dat=d,search=FALSE, ql=.01,qu=.99,obsvar="RT", #name of RT variable
             facs=c("condition1","condition2","subject"))  #design cell combos to fit

## 4. Plot the variables with within-subject error corrected bars 

swplotws(f$vars,c("Subj")) 
...
##################
## Example Application 2: Experiment 1 (Manual response task by baboons, Figure 6)

## 1. Load the data 
##    Note that the data file should be located in your "getwd()" folder 

d <- read.csv("AAV15_exp1.csv")

## 2. Fit the data

f <- swapply(dat=d,obsvar="RT",facs=c("condition","epoch","subject"),search=TRUE)

## note there are 2158 cells, thus fitting may take some time, alternatively, use:

f <- swapply(dat=d,obsvar="RT",facs=c("condition","epoch","subject"),search=FALSE)

## 3. Aggregate the 40 training levels into 8 levels for a simplified plot output 

f2 <- f
f2$vars$epoch <- factor(ceiling(as.numeric(f2$vars$epoch)/5))
d2 <- d; d2$epoch <- factor(ceiling(as.numeric(d2$epoch)/5))

## 4. Plot the variables with within-subject error corrected bars 

swplotws(f2$vars,c("subject")) 

## 5. Plot the model fit checks (that include all cells before they are aggregated)
